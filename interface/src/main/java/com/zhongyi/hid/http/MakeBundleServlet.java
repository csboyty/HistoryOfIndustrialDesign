...
public class MakeBundleServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest httpRequest,HttpServletResponse httpResponse) throws IOException{
		...
		try{
			...
			String[] temp=StringUtils.split(docId, ",");
			int [] docIdArray = new int[temp.length];
			for(int i=0;i<temp.length;i++){
				docIdArray[i] = Integer.parseInt(temp[i]);
			}
			bundleManageService.makeBundles(docIdArray,now);
		}catch(Exception ex){
			logger.error("bundle failed,docId:"+docId, ex);
			result = "failure";
		}
